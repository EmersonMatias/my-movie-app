        ...
        listMovies = {
            HomeScreen:
                [
                    ...
                    { title: "Popular TV Show", items: await (await axios.get(`${BASE_URL}tv/popular?api_key=${API_KEY}&language=en-US&page=1`)).data.results.concat(await (await axios.get(`${BASE_URL}movie/top_rated?api_key=${API_KEY}&language=en-US&page=2`)).data.results), margin: 120 },
            ...
            SeriesScreen:
            [
                {title:"Ação e Aventura", items: await (await axios.get(`${BASE_URL_TVDISCOVER}10759&page=1&include_video=true`)).data.results.concat(await (await axios.get(`${BASE_URL_TVDISCOVER}10759&page=2`)).data.results), margin:-240},
